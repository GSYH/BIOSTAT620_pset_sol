...
deaths_raw = deaths_raw|>
  ...
  mutate(death = parse_number(covid_19_deaths))|>
  mutate(date = as_date(ymd_hms(end_date)))|>
  mutate(mmwr_year = epiyear(date),mmwr_week = as.numeric(mmwr_week))|>
  select(state, mmwr_week, mmwr_year, death)|>
...
ggplot(vax_july, aes(x = state, percent_vacc, y = percent_vacc, fill = state)) +
