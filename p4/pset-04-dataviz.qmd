...
```{r}
library(lubridate)
cases_raw  = cases_raw |> select(start_date, state, new_cases) |>
  mutate(
    week = epiweek(start_date),
    year = epiyear(start_date),
    cases = as.integer(new_cases)
  ) |>
...
hosp_raw = hosp_raw |> select(jurisdiction, collection_date, hosp = new_covid_19_hospital) |>
  mutate(week = epiweek(collection_date),
         year = epiyear(collection_date)) |>
...
vax_raw = vax_raw |>
  ...
  mutate(
    booster = as.integer(booster_cumulative),
    series = as.integer(series_complete_cumulative)
  ) |>
  select(-date) |>
...
dat <- dates_and_pop %>%
  ...
  left_join(deaths_raw, by = c("state","mmwr_year","mmwr_week")) |>
