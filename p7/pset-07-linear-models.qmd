...
counts = puerto_rico_counts |>
  filter(date >= make_date(2002,1,1) & date <= make_date(2017,12,31))
  
counts = counts |>
  filter(agegroup %in% c("60-64", "65-69", "70-74", "75-79", "80-84", "85-Inf"))
```
...
```{r}
weekly_counts <- weekly_counts|>
  mutate(mortality_rate = (outcome / population) * 100000)



mortality_0216 = weekly_counts|>
  filter(year(date) >= 2002 & year(date) <= 2016)

mortality_17 = weekly_counts|>
...
```{r}
yearly_rates <- weekly_counts |> 
  filter(year(date) < 2017) |>  
  group_by(year = year(date)) |>  
  summarise(
    total_outcome = sum(outcome),  
    total_population = sum(population),  
    mortality_rate = (total_outcome / total_population) * 1000  
  )

ggplot(yearly_rates, aes(x = year, y = mortality_rate)) +
...
weekly_counts_model <- weekly_counts |> 
  filter(year(date) < 2017 & agegroup %in% c("65-69", "70-74", "75-79", "80-84", "85-Inf")) |> 
  mutate(
...
```{r}
weekly_counts = weekly_counts|>
  filter(year(date) < 2018 & agegroup %in% c("65-69", "70-74", "75-79", "80-84", "85-Inf"))
...
weekly_counts <- weekly_counts|>
  mutate(
    ...
    expected_outcome = (predicted_rate * population) / 1000,
    excess_deaths = outcome - expected_outcome,
    se_expected_outcome = (predictions$se.fit * population) / 1000,
...
ggplot(weekly_excess, aes(x = week, y = total_excess_deaths)) +
  geom_line()
...
ggplot(plot_data, aes(x = week, y = Rate, color = Rate_Type)) +
