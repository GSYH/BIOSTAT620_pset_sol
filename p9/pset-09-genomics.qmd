...
ggplot(section_df, aes(x = x)) +
  ...
  geom_point(aes(y = loess_smooth), color = "blue", size = 1) + theme_minimal()
```
...
pca_result <- prcomp(t(small_counts))

pc1 <- pca_result$x[, 1]

plot(pc1, cell_totals,pch = 16, col = "blue")
```
Comment on your finding:

This plot shows a strong linear relationship between: PC1 and total cellular count
...
I would say PC1 and PC2 appears to have statistically significant differences across gene types. For pc3, pc4 and pc5, we can see there is not a significant differences since both their CI are somehow close together overlap, like gene set in negative and other.
